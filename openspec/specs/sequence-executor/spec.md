# sequence-executor Specification
...
The executor SHALL emit `dtk-alea.complete` via `IHookEmitter` with the full `SequenceExecution` state and remove the entry from `ICombatStateStore` when all steps have executed (including chains, excluding suspended-await mid-sequence).
