...
The `DtkHubApi` interface SHALL expose the following contract via `game.dtk`:
...
The `AleaApi` interface SHALL expose the following contract for dice sequence execution:
...
The `LexApi` interface SHALL expose the following contract for expression evaluation and Codex management:
...
The `OpusApi` interface SHALL expose the following contract for character creation and advancement:
...
The `PromptariumApi` interface SHALL expose the following contract for corpus validation:
...
The `SystemaApi` interface SHALL expose the following contract for system registration:
