...
`OpusApi.openCreationWizard(actor: Actor, systemId: string): Promise<CharacterBuild | null>` SHALL look up the registered Forma for `systemId`, open the creation wizard UI, and resolve the Promise when the player completes (returns `CharacterBuild`) or cancels (returns `null`). Throws if no Forma is registered for the system.
