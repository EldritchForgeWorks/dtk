# exemplar-compiler Specification
...
The compiler SHALL write one LevelDB database per declared compendium pack (e.g., `species`, `rules`, `items`), with each entry stored as `key=!{type}!{id}`, `value=JSON(CompiledEntry)`.
