...
The `advancementSteps` array, when declared, SHALL conform to the same `WizardStep` schema as `creationSteps`. A Forma MAY omit this field; if omitted, dtk-opus disables advance mode for this system.
...
A `Condition` SHALL be a serialisable JSON predicate used for step/field visibility and
...
The `advancementTracks` array, when declared, SHALL contain `AdvancementEntry` objects representing purchasable abilities or improvements. A Forma MAY omit this field. The `AdvancementEntry` schema is:
