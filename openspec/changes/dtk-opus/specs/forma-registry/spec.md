...
Advancement tracks reference step ids from the same Forma's `steps[]`. The validator
SHALL check that any `unlocks_after` step id in an advancement track entry actually
exists in `steps[]`. Missing step id references SHALL produce a validation error.
