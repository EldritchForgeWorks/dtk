...
OpusApi is the public surface of dtk-opus, exposed via `game.dtk`. It is the entry
point for game systems to trigger creation and advancement flows.
...
If dtk-lex is not installed (`game.dtk.isInstalled('dtk-lex')` is false):
...
The full `OpusApi` interface exposed via `game.dtk`:
