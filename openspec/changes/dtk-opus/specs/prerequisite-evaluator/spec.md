...
If the prerequisite expression is beyond the built-in evaluator's capability (function
calls, comparisons, custom lookups), the evaluator SHALL call `ILexDelegate.evaluate(expr, context)`.
...
`PrerequisiteEvaluator.evaluateAll(forma: Forma, build: CharacterBuild): Record<string, boolean>`
SHALL evaluate all advancement prerequisite expressions and return a map of
`advancementId → boolean`. Used by the tracker to determine which advancements to show.
