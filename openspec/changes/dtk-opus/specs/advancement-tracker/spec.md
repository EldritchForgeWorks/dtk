# Spec: advancement-tracker

The advancement tracker is an ApplicationV2 panel that lets players spend their
advancement currency (XP, marks, resources, or milestone grants) to purchase
advancements defined in the Forma's advancement tracks. The paradigm that gates
advancement is declared in the Forma and stored in `CharacterBuild.paradigmState`.
...
The Buy button is gated by the Forma's declared `advancement.paradigm`. The engine
