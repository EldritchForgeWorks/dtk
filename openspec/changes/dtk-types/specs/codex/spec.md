...
The Codex is the contract a game system authors to define its vocabulary — the
named slugs that identify attributes, skills, derived stats, damage types, currencies,
and item categories. It is consumed by dtk-lex, which uses it to validate formula
references and rule expressions authored in compendium content.

## ADDED Requirements

### Requirement: CodexEntry — rich per-slug metadata

A Codex is composed of `CodexEntry` objects, not bare slug strings. Each entry
