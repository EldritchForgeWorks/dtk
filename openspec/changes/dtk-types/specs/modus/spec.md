...
The Modus is the contract a game system authors to wire itself into the DTK
ecosystem via a single `defineSystem()` call. It is consumed exclusively by
dtk-systema. The Modus is the top-level descriptor — it references the system's
Ritus, Codex, and Forma by value, and declares all Foundry-level registrations.
...
A Modus MAY declare an `items` record with the same shape as `actors`. If omitted
or empty, dtk-systema skips item registration.
...
A Modus MAY reference a `ritus` (consumed by dtk-alea), a `codex` (consumed by
dtk-lex), and a `forma` (consumed by dtk-opus). All three are optional — a system
that uses only dtk-systema and dtk-promptuarium need not declare them. Each is typed
as the corresponding contract type imported from `@dtk/types`.
...
A Modus MAY declare a `packs` array of compendium pack declarations consumed by
dtk-promptuarium's compiler. Each entry specifies one Foundry compendium pack to produce:
...
A Modus MAY declare a `types` array mapping Exemplar kinds to Foundry Document types
with field-level output mappers. Consumed by dtk-promptuarium's compiler to transform
Exemplar data into Foundry `system.*` fields:
...
A Modus MAY declare a `schemas` array of Alea roll schemas — supplementary schema
registrations beyond those embedded in the Ritus. MAY be empty or omitted.
...
A Modus MAY declare a `schemaVersion` string for world migration support. When
present, dtk-systema registers and runs migrations on the `ready` hook.
