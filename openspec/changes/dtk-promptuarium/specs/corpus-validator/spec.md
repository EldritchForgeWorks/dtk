...
For `kind: "discipline"` (requires `parent` archetype) and `kind: "vocation"` (requires
`parent` discipline), the validator SHALL check that the referenced id exists in the
corpus and is of the expected kind.
...
For `reference` grants (field `ref`) and `action` Exemplars (field `sequence`), the
validator SHALL check that the referenced id exists in the corpus. Kind checking is
