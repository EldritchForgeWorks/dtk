...
The NL generator produces `description` fields for `kind: rule`, `kind: sequence`,
and `kind: action` Exemplars that lack them. It uses Handlebars templates, resolves
Codex slugs to display names, and optionally calls an LLM for polish.
...
Templates reference Codex slugs via a `{{resolve slug}}` Handlebars helper. The helper
calls `ICodexProvider.resolveSlug(slug)` and returns the display name. If the slug is
