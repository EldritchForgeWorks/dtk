...
- **Stable manifest URL per module** via a per-module moving tag: each release also force-updates a `<module-id>-latest` tag/release, giving `https://github.com/EldritchForgeWorks/dtk/releases/download/<module-id>-latest/module.json` as the permanent `manifestUrl`. The versioned release's `module.json` `download` field points at the immutable versioned zip.
