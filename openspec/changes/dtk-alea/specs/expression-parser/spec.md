...
If the parser encounters an expression it cannot handle (conditionals, function calls,
logical operators, string operations), it SHALL call
`IExpressionDelegate.evaluate(expression, context)`. If the delegate returns a result,
