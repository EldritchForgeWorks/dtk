...
The Ritus registry stores registered Ritus configurations and provides the dice
mechanic, threshold, and tier config used by the roll resolver for a given system id.
...
Rule Exemplars may declare `mechanic`, `threshold`, and `tiers` overrides. The
`RitusRegistry.resolve(systemId, ruleOverrides)` method SHALL return a merged
`RitusConfig` with per-rule overrides applied on top of the base Ritus. The base
Ritus is not mutated.
