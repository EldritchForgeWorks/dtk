...
`defineSystem(modus: Modus)` is the primary entry point for game system authors using
DTK. Called once from the game system's `init` hook, it validates the Modus contract
and wires up all Foundry registration — actors, items, data models, settings, sockets,
and DTK hooks — from the declarative Modus config alone.
...
As part of its `init` hook handler, dtk-systema itself calls
`game.dtk.register({ id: 'dtk-systema', version, api: systemaApi })`. This happens
once, before the game system author calls `defineSystem()`. The `systemaApi` object
exposes `defineSystem`, `version`, and `isReady`.
...
After completing all async registration steps (including waiting for dtk-alea and
dtk-lex ready signals if applicable), dtk-systema SHALL fire
`Hooks.callAll('dtk-systema.ready')`. The hub then marks systema as ready in the
`dtk.ready` coordination flow.
