...
The action menu is a floating ApplicationV2 panel that opens when a token is
controlled on the canvas. It displays the actor's available actions derived from
compendium Exemplars of `kind: "action"`, evaluates their conditions, and triggers
the execution pipeline when clicked.
