...
The await relay handles the suspension and resumption of dtk-alea sequences when an
`await` step requires a player decision. It renders a decision dialog, relays the
request across the network via Foundry socket (GM-relay pattern), and persists pending
state to Combat flags so sequences survive client disconnect.
...
If the actor identified by `actorId` is owned by the current user (or is a GM-owned
actor and current user is GM), systema SHALL render an `ApplicationV2` decision dialog
directly — no socket relay needed. The dialog SHALL present the choices as labelled
