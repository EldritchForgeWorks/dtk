# Spec: expression-engine

The expression engine evaluates complex expression strings — conditionals, function
calls, logical operators — against a provided `ExpressionContext`. It is the backend
implementation for dtk-alea's `IExpressionDelegate` port.
