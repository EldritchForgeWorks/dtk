...
The update checker runs passively on each Foundry session (GM only). It compares
installed DTK module versions against the remote registry and surfaces a single
consolidated notification if any updates are available.
