# Spec: module-coordinator

The module coordinator is the runtime coordination layer. It exposes the `game.dtk`
namespace, collects registration and readiness signals from other DTK modules, and
emits `dtk.ready` when all active DTK modules have completed their initialisation.
It also implements `getDtkModuleApi` and `isDtkModuleInstalled` as declared in
`@dtk/types/apis`.
