...
The targeting workflow runs between action selection and dtk-alea execution. It
dispatches by mode, manages canvas interaction, and resolves to a `ResolvedTarget[]`
list. dtk-alea receives only the finished list — it never touches canvas hooks or
Foundry templates.
